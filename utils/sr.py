...
def pivot_zones(df: pd.DataFrame, order: int = 5, band: float = 0.002) -> pd.DataFrame:
    """
    Basit S/R bantları: yerel tepe/dip’leri al, yakın seviyeleri gruplayıp bant yap.
    band: fiyatın yüzdesi (0.2%) kadar bant genişliği başlatıcı.
    """
    ...
    if not levels:  # güvenlik
        return pd.DataFrame(columns=["kind","start","end","lo","hi"])

    # Yakın seviyeleri kümeler halinde birleştir
    ...
    for k, t, val in levels:
        if not cur_vals:
            ...
            continue
        ref = np.median([v[2] for v in cur_vals])
...
def tag_price_context(df: pd.DataFrame, zones: pd.DataFrame, tol: float = 0.002) -> pd.Series:
    """
    Fiyat, bir S/R bandına yakın mı? (yakınlık tol yüzdesi)
    """
    close = df["Close"]
...
def acceptance_rejection(df: pd.DataFrame, zones: pd.DataFrame, lookback: int = 3) -> pd.Series:
    """
    Basit acceptance/rejection algısı:
    - Rejection: bant içine iğne atıp, kapanış bant dışında (destekte yukarı, dirençte aşağı)
    - Acceptance: kapanış bant dışına çıkar ve 1-3 bar orada kalır
    """
    ...
    for ts in df.index:
        ...
        for _, z in zones.iterrows():
            ...
            if z.kind == "R":
                ...
                # rejection: band üstüne iğne ama alt kapanış
                if in_band and (px_c < z.lo) and (px_h > z.hi):
                    sig.loc[ts] = "reject_res"
                # acceptance: üstte kapanış ve sonraki barlar da üstte
