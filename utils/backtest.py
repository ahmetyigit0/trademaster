...
def simulate(df: pd.DataFrame, entries: pd.DataFrame, min_rr: float = 1.5) -> pd.DataFrame:
    """
    entries: columns -> entry_price, stop, target, side ('long'/'short')
    Basit bar-by-bar hedef/stop testi (ilk ulaşana say)
    """
    results = []
    for ts, row in entries.iterrows():
        ...
        if not rr_ok(row.entry_price, row.stop, row.target, min_rr):
            continue
        # ileri barlarda hedef/stop dokunma
        ...
        if hit is None:
            hit = ("open", future.index[-1])  # açık poz.
