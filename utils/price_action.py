...
def classify_spike_bias(df: pd.DataFrame) -> pd.Series:
    """
    Hacim spike geldiğinde, wick kompozisyonuna bak:
    - Dirençte uzun üst iğne -> satıcı ağırlığı (bearish)
    - Destekte uzun alt iğne -> alıcı ağırlığı (bullish)
    Basitleştirilmiş: üst iğne > alt iğne ise 'bearish', alt iğne > üst iğne ise 'bullish', aksi 'neutral'
    """
    w = wick_stats(df)
