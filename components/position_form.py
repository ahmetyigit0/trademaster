...
_SZ      = "pf__pos_size"    # session_state key: current size value
_SZ_VER  = "pf__ps_ver"      # session_state key: widget version counter (forces re-mount)
...
def _render_form(edit_id=None):
    ...
    # ── Unique state keys per form instance ──────────────────────────────────
    sz_key  = f"{_SZ}_{px}"
    ...
    with c1:
        symbol = st.text_input("Symbol", value=pos["symbol"] if editing else "",
                               placeholder="BTC, ETH...", key=f"{px}symbol").upper().strip()
    with c2:
    ...
    for i in range(num_entries):
        with ecols[i % len(ecols)]:
            ...
            p  = st.number_input(f"Entry {i+1}", min_value=0.0, value=ep, format="%.6f", key=f"{px}ep{i}")
            w  = st.number_input(f"Ağırlık {i+1} (%)", min_value=0.0, max_value=100.0, value=ew, key=f"{px}ew{i}")
            if p > 0:
            ...
            if calc["can_use_full"]:
                ...
                with ba:
                    if st.button("💡 Öneriyi Uygula", key=f"{px}apply"):
                        # ‼️ THE REAL FIX:
                        # We store the new value AND bump the version counter.
                        # The widget below uses ver as part of its key, so
                        # Streamlit treats it as a BRAND NEW widget on next
                        # render — value= is therefore honoured again.
                        st.session_state[sz_key]  = float(rec)
                        st.session_state[ver_key] = st.session_state.get(ver_key, 0) + 1
                        st.rerun()

    # Initialise defaults on first render
    if sz_key not in st.session_state:
        st.session_state[sz_key]  = float(pos.get("position_size", capital)) if editing else float(capital)
        st.session_state[ver_key] = 0

    # ── Position size widget ──────────────────────────────────────────────────
    # Key includes version → forces fresh mount after "Öneriyi Uygula"
    ...
    if calc.get("risk_per_unit") and position_size > 0:
        ...
        st.markdown(
            f"<span style='font-size:0.82rem;color:#8b949e'>Position Heat: "
    ...
    if save_clicked:
        ...
        record = dict(
            symbol=symbol, direction=direction, capital=capital, risk_pct=risk_pct,
