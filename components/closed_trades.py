...
def render_closed_trades(data: dict):
    ...
    st.markdown(
        ...
        f"grid-template-columns:44px 100px 80px 80px 120px 110px 80px 100px 100px;"
        f"gap:0;padding:8px 14px;background:{_DBG};border:1.5px solid {_DG};"
        f"border-radius:12px 12px 0 0;font-size:13px;font-weight:700;"
        f"color:{_DT};text-transform:uppercase;letter-spacing:0.08em'>"
        f"<div>#</div><div>Coin</div><div>Yön</div><div>Sonuç</div>"
        f"<div>PnL</div><div>R Multiple</div><div>R:R</div>"
        f"<div>Kapanış</div><div></div></div>",
...
def _render_trade_row(trade: dict):
    ...
    st.markdown(
        f"<div style='display:grid;"
        f"grid-template-columns:44px 100px 80px 80px 120px 110px 80px 100px 100px;"
        f"gap:0;padding:11px 14px;background:{row_bg};"
        ...
        f"color:{_DT}'>#{tid}</div>"
        f"<div style='font-weight:700;font-size:16px;color:{_TX}'>{symbol}</div>"
        f"<div><span style='background:{dir_bg};color:{dir_c};padding:4px 10px;"
        f"border-radius:7px;font-size:13px;font-weight:700'>{direction}</span></div>"
        f"<div><span style='background:{res_bg};color:{res_c};padding:4px 10px;"
        f"border-radius:7px;font-size:13px;font-weight:700'>{result}</span></div>"
        ...
        f"<div style='font-size:14px;color:{_DT}'>{rr_str}</div>"
        f"<div style='font-size:14px;color:{_DT}'>{closed_str}</div>"
        f"<div></div>"
        f"</div>", unsafe_allow_html=True)

    bc1, bc2, bc3 = st.columns([2, 1, 6])
    with bc1:
        lbl = "▲ Kapat" if is_open else "🔍 Detay"
